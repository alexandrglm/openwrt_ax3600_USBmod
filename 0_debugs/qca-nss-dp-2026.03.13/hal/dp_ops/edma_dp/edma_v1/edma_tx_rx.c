 ...
 * DEBUG
 *
 * 	Añadir decisiójn filtro de marca para mandar a CPU de forma optimizada
 * EN CONJUNCION CON ecm.ko ecm_classifiers_mark, si no, no tiene sentido
 * edma_clean_rx()
 *	Reap Rx descriptors
 */
							  ...
							  struct edma_rxdesc_ring *rxdesc_ring)
{
	...
	while (1) {
		...
		/* DEBUG: FILTRO DE MARCA 0x100 - INICIO */
		if (unlikely(skb->mark == 0x100)) {
			/* DEBUG: Obtener el dispositivo de origen */
			if (EDMA_RXPH_SRC_INFO_TYPE_GET(rxph) ==
				...
				goto next_rx_desc;
					...
					/* DEBUG: Configurar skb y enviar a CPU */
					...
					printk(KERN_INFO "EDMA: FILTERED packet with mark=0x100 to CPU (slow path)\n");

					netif_receive_skb(skb);
					goto next_rx_desc;
		}
		/* DEBUG: FILTRO DE MARCA 0x100 FIN */
