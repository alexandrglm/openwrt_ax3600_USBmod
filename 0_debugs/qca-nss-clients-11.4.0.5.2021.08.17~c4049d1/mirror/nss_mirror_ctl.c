...
static int nss_mirror_ctl_parse_reset_nexthop_cmd(char *buffer)
{
	...
	char *save_ptr;
	char *token;
	int found_mirrordev = 0;
	char buffer_copy[128];

	strscpy(buffer_copy, buffer, sizeof(buffer_copy));
	save_ptr = buffer_copy;
	...
	while ((token = strsep(&save_ptr, " ")) != NULL) {
		...
		*val = '\0';
		...
		if (!strcmp(key, "mirrordev")) {
			if (nss_mirror_ctl_get_netdev_by_name(val, &mirror_dev) < 0) {
				nss_mirror_warn("Error in getting mirror net device.\n");
				return -1;
			}
			found_mirrordev = 1;
		} else {
			nss_mirror_warn("Invalid param: %s in reset nexthop cmd\n", key);
			return -1;
		}
	}

	if (!found_mirrordev || !mirror_dev) {
		nss_mirror_warn("mirrordev parameter not found in reset_nexthop cmd\n");
		return -1;
	}
	...
	while ((token = strsep(&save_ptr, " ")) != NULL) {
		...
		*val = '\0';
		...
		if (!strcmp(key, "mirrordev")) {
			...
			found_mirrordev = 1;
		} else if (!strcmp(key, "mirror_next_hop")) {
			if (val[strlen(val) - 1] == '\n')
				val[strlen(val) - 1] = '\0';

			if (!strcmp(val, "eth_rx")) {
				nexthop_if_num = NSS_ETH_RX_INTERFACE;
			} else if (!strcmp(val, "n2h")) {
			...
			nss_mirror_warn("Invalid parameter: %s in set_nexthop cmd\n", key);
			return -1;
		}
	}

	if (!mirror_dev || !found_mirrordev) {
		nss_mirror_warn("Mirror device not found\n");
		return -1;
	}

	if (nexthop_if_num < 0 || !found_nexthop) {
...
static int nss_mirror_ctl_parse_disable_cmd(char *buffer)
{
	...
	if (!found_mirrordev || !mirror_dev) {
		nss_mirror_warn("mirrordev parameter not found in disable cmd\n");
		return -1;
	}
...
static int nss_mirror_ctl_parse_enable_cmd(char *buffer)
{
	...
	if (!found_mirrordev || !mirror_dev) {
		nss_mirror_warn("mirrordev parameter not found in enable cmd\n");
		return -1;
	}
...
static int nss_mirror_ctl_parse_display_cmd(char *buffer)
{
	...
	if (!found) {
		nss_mirror_warn("mirrordev parameter not found in display cmd\n");
		return -1;
	}
...
static int nss_mirror_ctl_parse_enable_ingress_pmc_cmd(char *buffer)
{
	...
	struct net_device *dev = NULL, *mirror_dev = NULL;
	...
	while ((token = strsep(&save_ptr, " ")) != NULL) {
		...
		*val = '\0';
		...
		if (!strcmp(key, "devname")) {
			if (nss_mirror_ctl_get_netdev_by_name(val, &dev) < 0) {
				if (mirror_dev)
					dev_put(mirror_dev);
	...
	if (!found_devname) {
		if (mirror_dev)
			dev_put(mirror_dev);
		nss_mirror_warn("devname parameter not found\n");
		return -1;
	}
	...
	if (!mirror_dev || !found_mirrordev) {
		if (mirror_dev)
			dev_put(mirror_dev);
		nss_mirror_warn("Mirror device is NULL or not found\n");
		return -1;
	}
	...
	while ((token = strsep(&save_ptr, " ")) != NULL) {
		...
		*val = '\0';
		...
		if (!strcmp(key, "devname")) {
			...
			nss_mirror_warn("Invalid param: %s in promiscuous disable cmd\n", key);
			return -1;
		}
	}

	if (!found_devname || if_num < 0) {
		nss_mirror_warn("devname parameter not found\n");
		return -1;
	}
...
static int nss_mirror_ctl_parse_destroy_cmd(char *buffer)
{
	...
	if (!found_mirrordev || !mirror_dev) {
		nss_mirror_warn("mirrordev parameter not found in destroy cmd\n");
		return -1;
	}
...
static int nss_mirror_ctl_parse_configure_cmd(char *buffer)
{
	...
	while ((token = strsep(&save_ptr, " ")) != NULL) {
		...
		*val = '\0';
		...
		if (!strcmp(key, "mirrordev")) {
			...
			found_mirrordev = 1;
		} else if (!strcmp(key, "mirror_size")) {
			if (nss_mirror_ctl_convert_char_to_u16(val, &mirror_size)) {
				nss_mirror_warn("Not able to parse %s to u16\n", val);
				return -1;
			}
		} else if (!strcmp(key, "mirror_point")) {
			if (nss_mirror_ctl_convert_char_to_u32(val, &mirror_point)) {
				nss_mirror_warn("Not able to parse %s to u32\n", val);
				return -1;
			}
		} else if (!strcmp(key, "mirror_offset")) {
			if (nss_mirror_ctl_convert_char_to_u16(val, &mirror_offset)) {
				nss_mirror_warn("Not able to parse %s to u16\n", val);
				return -1;
			}
		} else {
			nss_mirror_warn("Invalid parameter: %s in configure cmd\n", key);
			return -1;
		}
	}

	if (!found_mirrordev || !mirror_dev) {
		nss_mirror_warn("mirrordev parameter not found in configure cmd\n");
		return -1;
	}
	...
	if (nss_mirror_configure(mirror_dev, mirror_point, mirror_size, mirror_offset)) {
										 ...
										 void __user *buf, size_t *lenp, loff_t *ppos)
{
	...
	buffer = (char *)ctl->data;

	nextarg = nss_mirror_ctl_read_nextarg(&buffer);
	if (!nextarg) {
		nss_mirror_warn("No command found\n");
		return -EINVAL;
	}
	...
	switch (command) {
		...
		case NSS_MIRROR_CMD_CREATE: {
			...
			mirror_dev = nss_mirror_create();
			if (!mirror_dev) {
				nss_mirror_warn("nss_mirror_create failed\n");
				return -EINVAL;
			}
			nss_mirror_info("Mirror device %s created\n", mirror_dev->name);
			break;
		}

		case NSS_MIRROR_CMD_CONFIGURE:
			if (nss_mirror_ctl_parse_configure_cmd(buffer)) {
				nss_mirror_warn("configure cmd parsing failed\n");
				return -EINVAL;
			}
			break;
		...
		case NSS_MIRROR_CMD_SET_NEXTHOP:
			if (nss_mirror_ctl_parse_set_nexthop_cmd(buffer)) {
				nss_mirror_warn("set_nexthop cmd parsing failed\n");
				return -EINVAL;
			}
			break;
		...
		case NSS_MIRROR_CMD_RESET_NEXTHOP:
			if (nss_mirror_ctl_parse_reset_nexthop_cmd(buffer)) {
				nss_mirror_warn("reset_nexthop cmd parsing failed\n");
				return -EINVAL;
			}
			break;
		...
		case NSS_MIRROR_CMD_INGRESS_PMC_ENABLE:
			if (nss_mirror_ctl_parse_enable_ingress_pmc_cmd(buffer)) {
				nss_mirror_warn("ingress_pmc_enable cmd parsing failed\n");
				return -EINVAL;
			}
			break;
		...
		case NSS_MIRROR_CMD_INGRESS_PMC_DISABLE:
			if (nss_mirror_ctl_parse_disable_ingress_pmc_cmd(buffer)) {
				nss_mirror_warn("ingress_pmc_disable cmd parsing failed\n");
				return -EINVAL;
			}
			break;
		...
		case NSS_MIRROR_CMD_DISABLE:
			if (nss_mirror_ctl_parse_disable_cmd(buffer)) {
				nss_mirror_warn("disable cmd parsing failed\n");
				return -EINVAL;
			}
			break;
		...
		case NSS_MIRROR_CMD_ENABLE:
			if (nss_mirror_ctl_parse_enable_cmd(buffer)) {
				nss_mirror_warn("enable cmd parsing failed\n");
				return -EINVAL;
			}
			break;
		...
		case NSS_MIRROR_CMD_DISPLAY:
			if (nss_mirror_ctl_parse_display_cmd(buffer)) {
				nss_mirror_warn("display cmd parsing failed\n");
				return -EINVAL;
			}
			break;
		...
		case NSS_MIRROR_CMD_DESTROY:
			if (nss_mirror_ctl_parse_destroy_cmd(buffer)) {
				nss_mirror_warn("destroy cmd parsing failed\n");
				return -EINVAL;
			}
			break;
		...
		default:
			nss_mirror_warn("Unknown command: %s\n", nextarg);
			return -EINVAL;
	}
...
int nss_mirror_ctl_register(void)
{
	...
	if (!nss_mirror_ctl_header) {
		nss_mirror_warn("Creating tus putos muertos sysctl directory table header for mirror failed\n");
		return -1;
	}
	return 0;
}
